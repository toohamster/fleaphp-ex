...
### Response 响应封装（v2.3 门面模式）

```php
use FLEA\Response;

// 获取当前响应实例（单例，对标 Request::current()）
...
class PostController extends Action
{
    ...
    public function actionView(): ViewInterface
    {
        ...
        if (!$post) {
            throw new \FLEA\Exception\InvalidArguments('文章未找到');
        }

        return View::html('post/view.php', ['post' => $post]);
    }

    // 创建（GET 显示表单，POST 处理提交）
    public function actionCreate(): Response|ViewInterface
    {
    ...
    // 编辑
    public function actionEdit(): Response|ViewInterface
    {
        $id = $this->request->input('id');
        $post = $this->postModel->find($id);
    ...
    public function actionDelete(): Response
    {
        ...
        if (!$id) {
            return Response::error('文章 ID 不能为空', 400);
        }
...
**说明**：
- action 方法**返回** `ViewInterface` 或 `Response`，框架统一处理响应发送
- 不再使用 `$this->getView()->display()` 和 `echo`
- 短路中间件通过 `Response::current()->setView()` 设置响应后不调用 `$next()`
